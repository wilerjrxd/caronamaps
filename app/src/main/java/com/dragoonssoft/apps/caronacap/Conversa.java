...
public class Conversa extends AppCompatActivity {
    ...
    private String idOutroUsuario, nomeOutroUsuario, imgUrlOutroUsuario, telefoneOutroUsuario, nomeOutroUsuarioAux, imgUrlOutroUsuarioAux, telefoneOutroUsuarioAux;
    ...
    private ImageView nenhumaMensagem;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        alert_builder = new AlertDialog.Builder(Conversa.this);
                ...
                alert_builder.setMessage("Esta ação pode lhe custar dinheiro, uma vez que utiliza créditos que você tem com sua operadora celular.")
                    .setPositiveButton("CONTINUAR", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            if (ActivityCompat.checkSelfPermission(Conversa.this, Manifest.permission.CALL_PHONE) != PackageManager.PERMISSION_GRANTED) {
                                ...
                                ActivityCompat.requestPermissions(Conversa.this, new String[]{Manifest.permission.CALL_PHONE}, 1);
        ...
        barImgView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                //if(!idOutroUsuario.equals("Gqwzf627C0acZe6G48Ynqhtn7003")) {
                    Intent abrirPerfil = new Intent(Conversa.this, Perfil.class);
                    abrirPerfil.putExtra("idusuario_chat", idOutroUsuario);
                    abrirPerfil.putExtra("nomeusuario_chat", nomeOutroUsuario);
                    abrirPerfil.putExtra("imgusuario_chat", imgUrlOutroUsuario);
                    abrirPerfil.putExtra("telefoneusuario_chat", telefoneOutroUsuario);
                    abrirPerfil.putExtra("idusuario_corrente", mAuth.getCurrentUser().getUid());
        ...
        if(!idOutroUsuario.equals("Gqwzf627C0acZe6G48Ynqhtn7003")){
            ...
            barCallView.setVisibility(View.INVISIBLE);
        }
    ...
    private void carregarMensagens() {
        ...
        rootRef.child("mensagens").child(mAuth.getCurrentUser().getUid()).child(idOutroUsuario).addChildEventListener(new ChildEventListener() {
            ...
            public void onChildAdded(DataSnapshot dataSnapshot, String s) {
                //nenhumaMensagem.setVisibility(View.GONE);
    ...
    private void enviarMensagem() {
        ...
        String idUsuarioCorrente = mAuth.getCurrentUser().getUid();
        ConnectivityManager connMgr = (ConnectivityManager)getSystemService(Context.CONNECTIVITY_SERVICE);
        NetworkInfo networkInfo = connMgr.getActiveNetworkInfo();
        if(networkInfo != null && networkInfo.isConnected()){
            if(msg.length() != 0){
                //isNotificationAlreadySent = false;
                String usuarioCorrenteRef = "mensagens/" + idUsuarioCorrente + "/" + idOutroUsuario;
                String outroUsuarioRef = "mensagens/" + idOutroUsuario + "/" + idUsuarioCorrente;
                String conversaUsuarioCorrenteRef = "conversas/" + idUsuarioCorrente + "/" + idOutroUsuario;
                String conversaOutroUsuarioRef = "conversas/" + idOutroUsuario + "/" + idUsuarioCorrente;

                DatabaseReference outroUsuarioMsgPush = rootRef.child("mensagens").child(idUsuarioCorrente).child(idOutroUsuario).push();
                ...
                String horario = "-" + dateTime.get(Calendar.YEAR) + getMonthName() + corrigirDiaDoMes(dateTime.get(Calendar.DAY_OF_MONTH)) + corrigirFormatoHorarioSemDoisPontos(horas, minutos);
                ...
                msgMap.put("from", idUsuarioCorrente);
                ...
                conversaUsuarioCorrenteDataMap.put(conversaUsuarioCorrenteRef + "/nome", nomeOutroUsuario);
                conversaUsuarioCorrenteDataMap.put(conversaUsuarioCorrenteRef + "/img", imgUrlOutroUsuario);
                conversaUsuarioCorrenteDataMap.put(conversaUsuarioCorrenteRef + "/telefone", telefoneOutroUsuario);
                ...
                conversaOutroUsuarioDataMap.put(conversaOutroUsuarioRef + "/nome", nomeUsuarioCorrente);
                conversaOutroUsuarioDataMap.put(conversaOutroUsuarioRef + "/telefone", telefoneUsuarioCorrente);
                conversaOutroUsuarioDataMap.put(conversaOutroUsuarioRef + "/img", imgUrlUsuarioCorrente);
                
                rootRef.updateChildren(msgOutroUsuarioMap);
                rootRef.updateChildren(conversaUsuarioCorrenteDataMap);
                rootRef.updateChildren(conversaOutroUsuarioDataMap);
                sendNotification(online, idUsuarioCorrente);
    ...
    private void sendNotification(boolean online, String idUsuarioCorrente) {
            if (!online || !outroUsuarioConversandoCom.equals(idUsuarioCorrente)) {
                HashMap<String, String> notificationInfo = new HashMap<>();
                notificationInfo.put("from", idUsuarioCorrente);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == android.R.id.home) {
            HomeTabbed.fragmentIndexToSelect = 1;
        }
    ...
    public void onBackPressed() {
        //if(veioDeDealWithCarona) {
            HomeTabbed.fragmentIndexToSelect = 1;
        //}else{
        //    HomeTabbed.fragmentIndexToSelect = 2;
        //}
        //MensagensFragmento.mensagensJaCarregadas = false;
